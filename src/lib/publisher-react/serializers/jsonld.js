...
function compactPerson(c) {
  if (!c) return null
  const name = c.name || [c.given, c.family].filter(Boolean).join(' ').trim() || undefined
  const aff = c.affiliation
    ? typeof c.affiliation === 'string'
      ? {
        '@type': 'Organization',
        name: c.affiliation,
      }
      : {
        '@type': 'Organization',
        name: affName(c.affiliation),
        ...(c.affiliation.unitCode ? { identifier: c.affiliation.unitCode } : {}),
        ...(c.affiliation.unitUri ? { url: c.affiliation.unitUri } : {}),
      }
    : undefined
  return {
    '@type': 'Person',
